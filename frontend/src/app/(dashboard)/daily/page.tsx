...
export default function DailyPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["dashboard-daily", startDate, endDate],
    queryFn: async () => (await api.get("/dashboard/daily", { params: { start_date: startDate, end_date: endDate } })).data,
  });

  return (
    <div>
      <h2 className="text-2xl font-bold mb-6">Dashboard Harian</h2>
            ...
            <Table>
              ...
              <TableBody>
                {(data || []).map((row: any, i: number) => (
                  <TableRow key={i}>
                    <TableCell className="font-medium">{row.trx_date}</TableCell>
                ...
                {(!data || data.length === 0) && (
                  <TableRow><TableCell colSpan={8} className="text-center text-slate-400">No data</TableCell></TableRow>
                )}
              </TableBody>
            </Table>
          </CardContent>
